...
## 11. 결정된 설계 사항 (2026-07-10 확정 · 추천안 채택)

1. **모듈 진입점** — **독립 모듈**. 사이드바 모듈 스위처를 3개로 확장(협업 스타일 / WorkCraft / Working Backwards).
2. **페르소나 데이터 범위** — 실제 동료는 **공개 협업 스타일 리포트만** 페르소나로 사용(비공개 보호). 미완료/비공개자는 역할 프리셋만.
3. **Pain 가져오기 소스** — 팀 **공유 불편함(team_public)** + **회고 트렌드(익명 카테고리)** + 본인 개인 불편함. 타인의 비공개는 불가.
4. **검증 점수 자동 힌트** — `반복성`·`이해관계자 수`·`데이터 축적 가치`만 데이터로 제안(사용자 조정 가능), 나머지는 수동.
5. **출력 형식** — Phase 1은 **Markdown**(미리보기+복사+다운로드). DOCX/PPTX는 Phase 4.
6. **LLM 브릿지** — **Phase 1에 포함**. export 시 "이 skeleton을 다듬어 달라"는 붙여넣기용 프롬프트를 함께 생성.
