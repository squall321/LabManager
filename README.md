...
첫 번째 모듈은 **협업 스타일 워크샵** — 한글 설문으로 팀 내 협업 방식·업무 선호를 탐색하고, 함께 일할 때 서로 배려할 점을 정리해주는 자체 활동입니다.

> ⚠️ 본 워크샵은 특정 상용 진단도구의 공식 프로그램이 아니며, 팀 내 협업 방식과 업무 선호를 탐색하기 위한 자체 활동입니다. 개인 평가나 인사 판단에 사용하지 않습니다.
...
1. **설문 (100문항, 3섹션)** — 문항·채점 규칙은 [birkman_data.py](backend/app/services/birkman_data.py) 단일 소스에서 관리
   - 섹션 1: 나의 행동과 성향 (자기 평가, 36문항 · 11개 스타일 요소 전체 커버)
   - 섹션 2: 사람과 환경에 대한 생각 (타인 인식 → 편한 조건, 32문항 · 일부 역채점)
   - 섹션 3: 나의 관심 영역 (흥미, 32문항 · 8개 영역 균등)
2. **자동 분석** → 4색상 스타일(빨강 추진형/초록 관계형/노랑 체계형/파랑 전략형), 협업 스타일 맵(드러나는 스타일·편한 조건 **이중 좌표**), 11개 스타일 요소, 8개 관심 영역, 강도(intensity)
3. **리포트** → **함께 일하기 가이드(동료가 나와 일할 때 / 내가 동료를 대할 때)** + 드러나는 스타일·편한 조건·삐걱일 때·팀 역할·소통법·성장 포인트
